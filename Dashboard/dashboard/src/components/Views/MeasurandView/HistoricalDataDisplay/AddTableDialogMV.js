...
import TableFormMV from "./TableFormMV";
...
const AddTableDialog = ({
  ...
  // Initialize form data with initialData when dialog opens
  useEffect(() => {
    if (open) {
      if (initialData) {
        setFormData({
          ...
          terminals: initialData.terminals || [],
  ...
  const handleSubmit = async () => {
    ...
    if (isValid) {
      ...
      try {
        // Generate table name if not provided: profilename-measurand-terminalscount
        const tableName =
          formData.name ||
          `${formData.profile}-${formData.measurand}-${formData.terminals.length}`;
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Select a Profile first, then a Plant, followed by one Measurand,
              and finally one or more Terminals to create your data table.
            </Typography>
          </Paper>
        </Box>
